...
def pedidos_cliente(arr_cliente, arr_cantidades):
    totales_cliente = [0] * 6 # Inicializar el arreglo de acumuladores para los clientes 10 al 15.
    i = 0
    while i < len(arr_cliente):
        cod_cliente = arr_cliente[i]
        cantidad = arr_cantidades[i] 
        indice = cod_cliente - 10 # Calculamos el índice (Mapeo: 10->0, 11->1, ..., 15->5)
        if 0 <= indice < 6: # Validamos que el índice pertenezca al rango 0-5 antes de sumar
            totales_cliente[indice] += cantidad
        i += 1  
    
    print(f"{'Cliente':<14}{'Banquetas':>14}")
    print("-----------------------------")
    j = 0
    while j < 6: 
        cliente_id = j + 10
...
def calcular_porc_cliente(arr_cliente, arr_cantidades, total_general):
...
def calcular_porc_modelo(arr_modelos, arr_cantidades, total_general):
    #Calcula y muestra el porcentaje de banquetas de cada modelo.
    if total_general == 0:
        print("\nNo hay banquetas para calcular porcentajes por modelo.")
        return
...
if len(cantidades) > 0:
    ...
    print("\nF. Porcentaje de cada modelo:\n")
    calcular_porc_modelo(modelos, cantidades, total_banquetas)
else:
